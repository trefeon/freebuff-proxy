...
> (Premium pool `5/day` `pacific_day` `America/Los_Angeles`; shared by `GPT-5.6 Luna` + `Solar Pro 4` (solar additionally `1/day` `pool:solar_pro4` cap on top of the shared pool). `GLM 5.3 Flash` is **unmetered** — no per-model cap, left the premium pool `2026-08-28`.)
...
| **Unlimited**| **GLM 5.3 Flash** `NEW` | `z-ai/glm-5.3-flash` | **Deep reasoning**, Images. **Unmetered** — always available, no per-model cap (left the premium pool `2026-08-28`; now the default pick). |
