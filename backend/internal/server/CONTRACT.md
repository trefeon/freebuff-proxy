...
- Admin: `admin.go`, `admin_auth.go` (HMAC cookie, login rate limit, loopback gates), `admin_env.go` (config editor), `admin_settings.go` (ADR-0019 DB overlay: `loadConfig` is the single overlay-aware reload every admin mutation funnels through; GET/POST/DELETE `/admin/api/settings` — GET carries `degraded:true` when the store is nil, POST coerces integral JSON numbers for int keys and reports catalog `RestartOnly` as `setting_restart_only`), `admin_pages.go` (per-page UI snapshots over the same store handle; GET/PUT `/admin/api/pages/{id}`, allowlisted ids — all 12 SPA nav-registry ids plus the `shell` chrome key — PUT data must be a JSON object (400 otherwise), 64KB data cap with envelope-over-limiter 413s as `page_too_large`, nil-store GET-degrades/PUT-503s), `admin_handlers.go`, `admin_tokens*.go` (token management; mode-switch and require-login shadow errors name the DB overlay when it is the true blocker via `ove…
