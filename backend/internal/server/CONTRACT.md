...
- Admin: `admin.go`, `admin_auth.go` (HMAC cookie, login rate limit, loopback gates), `admin_env.go` (config editor), `admin_settings.go` (ADR-0019 DB overlay: `loadConfig` is the single overlay-aware reload every admin mutation funnels through; GET/POST/DELETE `/admin/api/settings`), `admin_pages.go` (per-page UI snapshots over the same store handle; GET/PUT `/admin/api/pages/{id}`, allowlisted ids, 64KB cap, nil-store GET-degrades/PUT-503s), `admin_handlers.go`, `admin_tokens*.go` (token management), `admin_maturity.go`, `admin_csrf_test.go` neighbors.
