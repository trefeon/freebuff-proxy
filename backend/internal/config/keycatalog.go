...
var keyCatalog = []KeyDef{
	...
	{Key: "LISTEN_ADDR", Group: GroupGeneral, Kind: "text", Hidden: true,
		...
		Description: `Host and port to bind (loopback by default; containers set :3457 for all interfaces). Set via container environment.`},
	...
	{Key: "MATURITY_ALLOW_PREMIUM", Group: GroupPool, Kind: "bool",
		Default:     "false",
		Description: `Allow the per-token premium-short touch mode (one short premium admission instead of the Economy touch). Default false: premium touches spend the account's daily Freebucks pool and stay opt-in per token.`},
