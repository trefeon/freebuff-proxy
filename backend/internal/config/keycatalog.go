...
var keyCatalog = []KeyDef{
	...
	{Key: "QUOTA_AUTO_PROBE", Group: GroupPool, Kind: "bool",
		Default:     "true",
		Description: `Keep per-token quota fresh without manual probes: each pooled token is probed once per Pacific day at a deterministic jittered slot in the 2h window before its known quota reset (ADR-0022). Applies live on reload; false restores pre-scheduler behavior.`},
