...
var keyCatalog = []KeyDef{
	...
	{Key: "MATURITY_ENABLED", Group: GroupPool, Kind: "bool", Essential: true,
		Default:     "true",
		Description: `Global kill-switch for the nightly streak-maintenance run: enrolled tokens stay leasable and get one low-cost touch in the 60 minutes before the Pacific-midnight reset until they reach MATURITY_TARGET_DAYS. Default true (probes run in dry-run mode); set false to disable all automated touches.`},
	{Key: "MATURITY_TARGET_DAYS", Group: GroupPool, Kind: "int",
		Default:     "7",
		Description: `Global streak target for every enrolled token (1-28 days; the dashboard enrolls with target 0 = this default, per-account targets are gone). A token whose streak reaches the target stops getting touches (automation disables itself).`},
