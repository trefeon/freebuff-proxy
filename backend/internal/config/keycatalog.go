...
var keyCatalog = []KeyDef{
	...
	{Key: "MATURITY_TOUCH_MODEL", Group: GroupPool, Kind: "text",
		Default:     "auto",
		Description: `Global touch model for the nightly run: "auto" (default) admits the cheapest served price-0/exempt row per account and falls back to this key's explicit value when no unmetered served row exists; an explicit provider/model id overrides auto. The touch never spends Freebucks (fail-closed on priced rows). Accounts with client traffic since the last Pacific reset are skipped, as are quarantined/banned/cooling/country-blocked ones.`},
