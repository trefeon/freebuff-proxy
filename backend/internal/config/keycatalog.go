...
var keyCatalog = []KeyDef{
	...
	{Key: "CHAT_MAX_INFLIGHT_METERED", Group: GroupPool, Kind: "int", Hidden: true,
		Default:     "1",
		Description: `Cap on concurrent in-flight chat requests per token on metered (priced Freebucks) models (0 = unlimited).`},
	{Key: "CHAT_MAX_INFLIGHT_UNMETERED", Group: GroupPool, Kind: "int", Hidden: true,
		Default:     "3",
		Description: `Cap on concurrent in-flight chat requests per token on unmetered (unpriced) models (0 = unlimited).`},
	...
	{Key: "TOKEN_MAX_CONCURRENT", Group: GroupPool, Kind: "int",
		Default:     "2",
		Description: `Cap on concurrent live turns per pooled token (floor 1: lower values floor to 1, a zero live-turn cap could never serve). A lease is granted only while the token holds fewer live turns; excess waiters park FIFO until QUEUE_WAIT elapses. Applies live on reload. BUNKER PRESET: 1 — fully sequential turns per account, zero parallel fingerprint.`},
	...
	{Key: "MAX_REQUESTS_PER_DAY", Group: GroupQuota, Kind: "int",
		Default:     "1500",
		Description: `Per-token cap on successful chat requests per Pacific day (0 = unlimited). Locks the account until the daily quota reset (Pacific midnight, same instant upstream rolls its windows); capped tokens are skipped and the pool rolls to the next account.`},
	{Key: "MAX_REQUESTS_PER_MINUTE", Group: GroupQuota, Kind: "int",
		Default:     "30",
		Description: `Per-token cap on admitted chat requests per rolling 60s window (0 = unlimited). Throttles upstream-visible request bursts so each account stays under abuse-detection rates; a runaway loop locks within a minute and the pool rolls to the next account.`},
