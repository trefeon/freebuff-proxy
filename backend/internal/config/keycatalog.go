...
type KeyDef struct {
	...
	// Secret marks keys whose effective value must be masked in the
	// dashboard (they may carry credentials).
	Secret bool `json:"secret"`
	// Essential marks keys shown by default in the dashboard Settings UI.
...
var keyCatalog = []KeyDef{
	// ── general ──────────────────────────────────────────────────────────
	{Key: "ACTING_USER_ID", Group: GroupGeneral, Kind: "text", RestartOnly: true,
		Default:     "",
	...
	{Key: "AUTO_DISCOVER_TOKEN", Group: GroupGeneral, Kind: "bool", Essential: true,
		Default:     "true",
	...
	{Key: "CLI_VERSION", Group: GroupGeneral, Kind: "text",
		Default:     "0.10.7",
		Description: `Upstream CLI version string; informational only — parsed and shown on the dashboard, no wire impact.`},
	{Key: "DASHBOARD_ENABLED", Group: GroupGeneral, Kind: "bool", Essential: true,
		Default:     "true",
		Description: `Serve the embedded admin dashboard at /admin (false disables all /admin routes with 404).`},
	...
	{Key: "LISTEN_ADDR", Group: GroupGeneral, Kind: "text", Essential: true,
	...
	{Key: "LOG_FILE", Group: GroupGeneral, Kind: "text", Essential: true,
		Default:     "",
		Description: `Append log lines to a file (e.g. ./logs/proxy.log); empty = stderr.`},
	{Key: "LOG_FORMAT", Group: GroupGeneral, Kind: "select", Enum: []string{"text", "json"},
		Default:     "text",
	...
	{Key: "LOG_RING_SIZE", Group: GroupGeneral, Kind: "int",
		Default:     "500",
	...
	{Key: "API_KEYS", Group: GroupPool, Kind: "list", Secret: true,
		Default:     "",
		Description: `Comma-separated client keys required for /v1/* (empty = open; ignored in bridge mode). In hybrid mode API_KEYS is the discriminator: a matching credential uses the pool, any other is relayed upstream as a bridge token.`},
	{Key: "AUTH_TOKENS", Group: GroupPool, Kind: "list", Secret: true,
		Default:     "",
		Description: `Comma-separated upstream FreeBuff tokens. EMPTY = bridge mode: the proxy stores no tokens and each client sends its own; set = pooled (or hybrid with BRIDGE_ENABLED).`},
	...
	{Key: "ROTATION_INTERVAL", Group: GroupPool, Kind: "text", RestartOnly: true,
		Default:     "6h",
		Description: `Agent-run rotation interval.`},
	{Key: "RUNS_DRAIN_QUEUE_CAP", Group: GroupPool, Kind: "int", RestartOnly: true,
		Default:     "64",
		Description: `Draining-runs list cap; entries beyond it are force-dropped (FINISH is best-effort).`},
	{Key: "RUNS_DRAIN_TTL", Group: GroupPool, Kind: "text", RestartOnly: true,
		Default:     "10m",
		Description: `Draining-runs TTL eviction window.`},
	{Key: "RUN_FINISH_INLINE_TIMEOUT", Group: GroupPool, Kind: "text", RestartOnly: true,
		Default:     "250ms",
		Description: `Synchronous inline FINISH fallback bound when the deferred-FINISH queue is full.`},
	{Key: "RUN_FINISH_QUEUE_SIZE", Group: GroupPool, Kind: "int", RestartOnly: true,
		Default:     "64",
	...
	{Key: "SESSION_RE_ADMIT_LEAD", Group: GroupPool, Kind: "text",
		Default:     "60s",
	...
	{Key: "SESSION_STATE_FILE", Group: GroupPool, Kind: "text", RestartOnly: true,
	...
	{Key: "TOKEN_ROTATION", Group: GroupPool, Kind: "select", Enum: []string{"drain", "round_robin", "least_used", "random"}, Essential: true,
		Default:     "drain",
		Description: `Token selection strategy: drain (exhaust a token's session before rotating), round_robin, least_used, or random.`},
	{Key: "WAITING_ROOM_CHAIN", Group: GroupPool, Kind: "bool",
		Default:     "false",
	...
	{Key: "FALLBACK_AFTER_MS", Group: GroupQuota, Kind: "int", Essential: true,
		Default:     "10000",
		Description: `Queue-wait threshold (ms) before re-routing to FALLBACK_MODEL; 0 disables fallback. Queue-wait only — never on 429 quota exhaustion.`},
	{Key: "FALLBACK_MODEL", Group: GroupQuota, Kind: "list",
		Default:     "",
		Description: `Map model=fallback (comma-separated) to re-route a request when its queue wait passes FALLBACK_AFTER_MS. Unset default: openai/gpt-5.6-luna → deepseek/deepseek-v4-flash.`},
	{Key: "MAX_MESSAGES_PER_DAY", Group: GroupQuota, Kind: "int", Essential: true,
		Default:     "0",
	...
	{Key: "QUOTA_FALLBACK_MODELS", Group: GroupQuota, Kind: "list",
	...
	// ── upstream ─────────────────────────────────────────────────────────
	{Key: "COST_MODE", Group: GroupUpstream, Kind: "select", Enum: []string{"free"}, RestartOnly: true, Essential: true,
		Default:     "free",
		Description: `MUST stay "free": any other value routes requests as PAID and fresh free accounts get 402 "Out of credits".`},
	{Key: "HTTP2_UPSTREAM", Group: GroupUpstream, Kind: "bool", RestartOnly: true,
		Default:     "true",
	...
	{Key: "MODEL_ALIASES", Group: GroupUpstream, Kind: "list",
		Default:     "",
	...
	{Key: "REQUEST_JITTER", Group: GroupUpstream, Kind: "text", RestartOnly: true,
		Default:     "0s",
		Description: `Random delay range [0, REQUEST_JITTER) before upstream chat calls; SAFE_MODE sets 2s when unset; 0 disables.`},
	{Key: "REQUEST_TIMEOUT", Group: GroupUpstream, Kind: "text", RestartOnly: true,
		Default:     "15m",
		Description: `Upstream request timeout.`},
	{Key: "SESSION_CALL_TIMEOUT", Group: GroupUpstream, Kind: "text", RestartOnly: true,
		Default:     "30s",
	...
	{Key: "TLS_FINGERPRINT", Group: GroupUpstream, Kind: "select",
		...
		RestartOnly: true, Default: "auto", Essential: true,
	...
	{Key: "UPSTREAM_BASE_URL", Group: GroupUpstream, Kind: "text", RestartOnly: true,
	...
	{Key: "CORS_ALLOWED_ORIGIN", Group: GroupSecurity, Kind: "text", Essential: true,
		Default:     "*",
		Description: `Access-Control-Allow-Origin for /v1/* responses.`},
	{Key: "WEBHOOK_URL", Group: GroupSecurity, Kind: "text", Secret: true, RestartOnly: true, Essential: true,
		Default:     "",
