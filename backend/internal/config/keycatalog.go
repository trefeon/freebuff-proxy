...
var keyCatalog = []KeyDef{
	...
	{Key: "LOG_CONSOLE_WINDOW", Group: GroupGeneral, Kind: "text",
		Default:     "1h",
		Description: `Default time window of the Activity console (Go duration, e.g. 15m, 1h, 24h). VIEW ONLY: it bounds which history rows the console asks for and never changes what is stored, so widening it reveals older entries without any reload. Zero or invalid falls back to 1h.`},
	...
	{Key: "LOG_TABLE_RETENTION", Group: GroupGeneral, Kind: "text",
		Default:     "168h",
		Description: `Storage retention for the log_entries and request_records history tables (Go duration; default 168h = 7 days). This is what the purge actually deletes: rows older than this are gone for good, so keep it at least as long as the console window you want to read. Quota and maturity history keep their own 90-day retention. Zero or invalid falls back to 168h.`},
