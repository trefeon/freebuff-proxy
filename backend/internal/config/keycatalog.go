...
var keyCatalog = []KeyDef{
	...
	{Key: "DEBUG_DUMP", Group: GroupGeneral, Kind: "bool", RestartOnly: true,
		Default:     "false",
		Description: `Persist redacted traffic dumps to ./dump/ (mode 0600, debugging only).`},
	{Key: "DEVTOOLS_ENABLED", Group: GroupGeneral, Kind: "bool",
		Default:     "false",
	...
	{Key: "LISTEN_ADDR", Group: GroupGeneral, Kind: "text", RestartOnly: true, Hidden: true,
		Default:     "127.0.0.1:3457",
		Description: `Host and port to bind (loopback by default; containers set :3457 for all interfaces). Set via container environment. The listener socket is bound once at boot and never re-binds on reload.`},
	{Key: "LOG_ACCESS", Group: GroupGeneral, Kind: "bool",
		Default:     "true",
		Description: `Log one access line per HTTP request (healthz/metrics/OPTIONS are rate-limited to 1/min regardless).`},
	{Key: "LOG_FILE", Group: GroupGeneral, Kind: "text", RestartOnly: true, Hidden: true,
		Default:     "",
		Description: `Append log lines to a file (e.g. ./logs/proxy.log); empty = stderr. Restart-only: the log sink is opened once at boot and a reload never re-opens it.`},
	{Key: "LOG_FORMAT", Group: GroupGeneral, Kind: "select", Enum: []string{"text", "json"}, RestartOnly: true,
		Default:     "text",
		Description: `Log format: text (key=value, colored) or json (one JSON object per line). Restart-only: the logger is configured once at boot and a reload never reconfigures it.`},
	...
	{Key: "LOG_RING_SIZE", Group: GroupGeneral, Kind: "int", RestartOnly: true,
		Default:     "500",
		Description: `In-memory log ring capacity behind the dashboard log viewer (50-5000). Restart-only: the ring is allocated once at boot and a reload never resizes it.`},
	...
	{Key: "BURST_BALANCE_ENABLED", Group: GroupPool, Kind: "bool",
		Default:     "true",
		Description: `Spread one model's request burst across up to BURST_MAX_TOKENS accounts once BURST_THRESHOLD same-model admissions land inside BURST_WINDOW. Applies live on reload; false restores drain-only selection. ANTI-BAN TRADEOFF: spreading looks less like single-user traffic than drain — set false only when strict single-account sequencing matters more than burst throughput.`},
	{Key: "BURST_MAX_TOKENS", Group: GroupPool, Kind: "int",
		Default:     "2",
		Description: `Cap on the distinct accounts one model's burst spreads across (minimum 2). Applies live on reload. Keep small: every extra account in the burst widens the multi-account fingerprint.`},
	{Key: "BURST_THRESHOLD", Group: GroupPool, Kind: "int",
		Default:     "20",
		Description: `Same-model admissions inside BURST_WINDOW that trip burst spreading for that model (other models keep the configured strategy). Applies live on reload. Lower values spread sooner — and look less single-user sooner.`},
	{Key: "BURST_WINDOW", Group: GroupPool, Kind: "text",
		Default:     "1m",
		Description: `Sliding window for counting same-model admissions toward BURST_THRESHOLD (Go duration, e.g. 30s, 1m, 5m). Applies live on reload. Shorter windows trip on sharper spikes; longer windows ride sustained load.`},
	{Key: "CHAT_MAX_INFLIGHT_METERED", Group: GroupPool, Kind: "int",
		Default:     "0",
		Description: `Cap on concurrent in-flight chat requests per token on metered (priced Freebucks) models (0 = unlimited, the default — the upstream quota/429 is the natural brake). Set a value to bound a burst locally.`},
	{Key: "CHAT_MAX_INFLIGHT_UNMETERED", Group: GroupPool, Kind: "int",
		Default:     "0",
		Description: `Cap on concurrent in-flight chat requests per token on unmetered (unpriced) models (0 = unlimited, the default — the upstream quota/429 is the natural brake). Set a value to bound a burst locally.`},
	...
	{Key: "MATURITY_DRY_RUN", Group: GroupPool, Kind: "bool",
		Default:     "true",
		Description: `Streak-maintenance touches run only the zero-cost session probe and never claim a session slot (nightly-run mechanics validation). Set false after the dry-run log lines prove the window, skips and throttles behave.`},
	{Key: "MATURITY_ENABLED", Group: GroupPool, Kind: "bool", Essential: true,
		Default:     "true",
		Description: `Global kill-switch for the nightly streak-maintenance run (the ONLY streak control): every account is touched automatically once nightly in the 60 minutes before the Pacific-midnight reset. Default true (probes run in dry-run mode); set false to disable all automated touches.`},
	...
	{Key: "MATURITY_TOUCH_MODEL", Group: GroupPool, Kind: "text",
		Default:     "",
		Description: `Global touch model for the nightly run: empty (default, same as "auto") admits the cheapest served price-0/exempt row per account and falls back to this key's explicit value when no unmetered served row exists; an explicit provider/model id overrides auto. The touch never spends Freebucks (fail-closed on priced rows). Accounts with client traffic since the last Pacific reset are skipped, as are quarantined/banned/cooling/country-blocked/locked ones.`},
	{Key: "MODEL_LOCKS", Group: GroupPool, Kind: "text",
		Default:     "",
		Description: `Pin pool slots to models (slot-indexed allowlist, e.g. "0:z-ai/glm-5.2;1:upstage/solar-pro4,mimo/mimo-v2.5"). Slots without an entry serve any model. Applies live on reload; malformed values reject the config.`},
	...
	{Key: "QUOTA_AUTO_PROBE", Group: GroupPool, Kind: "bool",
		Default:     "true",
		Description: `Master switch for the activity-aware quota prober: a busy pool probes every QUOTA_PROBE_ACTIVE_INTERVAL, a warming pool every 5m, an idle pool once plus the QUOTA_PROBE_IDLE_HEARTBEAT heartbeat. Applies live on reload; false restores pre-scheduler behavior (manual probes only).`},
	{Key: "QUOTA_PROBE_ACTIVE_INTERVAL", Group: GroupPool, Kind: "text",
		Default:     "60s",
		Description: `How often each pooled token is quota-probed while the pool is busy (traffic under 2m ago). Go duration (default 60s); the warming cadence stays 5m, idle is covered by QUOTA_PROBE_IDLE_HEARTBEAT. Applies live on reload.`},
	{Key: "QUOTA_PROBE_IDLE_HEARTBEAT", Group: GroupPool, Kind: "text",
		Default:     "30m",
		Description: `How often each pooled token is quota-probed while the pool sits idle (no traffic for 15m or more): one probe fires when traffic stops, then only this heartbeat until traffic resumes. Go duration (default 30m, also the ceiling for 429-backoff doubling). Applies live on reload.`},
	...
	{Key: "SESSION_CREATE_MAX_PARALLEL_GLOBAL", Group: GroupPool, Kind: "int",
		Default:     "0",
		Description: `Cap on concurrent in-flight session admissions (wait-or-503) across all models (0 = unlimited, the default — the upstream quota/429 is the natural brake). Set a value to bound a burst locally.`},
	{Key: "SESSION_CREATE_MAX_PARALLEL_PER_MODEL", Group: GroupPool, Kind: "int",
		Default:     "0",
		Description: `Per-model cap on concurrent in-flight session admissions (0 = unlimited, the default — the upstream quota/429 is the natural brake). Set a value to bound a burst locally.`},
	...
	{Key: "TOKEN_MAX_CONCURRENT", Group: GroupPool, Kind: "int",
		Default:     "2",
		Description: `Cap on concurrent live turns per pooled token (default 2, the approved anti-ban pacing; 0 = unlimited, no slot gating at all). A lease is granted only while the token holds fewer live turns; excess waiters park FIFO until QUEUE_WAIT elapses. Applies live on reload. BUNKER PRESET: 1 — fully sequential turns per account, zero parallel fingerprint.`},
	...
	// ── quota ────────────────────────────────────────────────────────────
	{Key: "BRIDGE_DAILY_LIMIT", Group: GroupQuota, Kind: "int",
		Default:     "0",
		Description: `Global daily chat cap across ALL bridge-mode entries (0 = unlimited).`},
	{Key: "FALLBACK_AFTER_MS", Group: GroupQuota, Kind: "int",
		Default:     "0",
	...
	{Key: "MAX_MESSAGES_PER_DAY", Group: GroupQuota, Kind: "int",
		Default:     "0",
		Description: `Per-token daily cap on successful chats (0 = unlimited; the upstream 429 lock is the real enforcement).`},
	{Key: "MAX_REQUESTS_PER_DAY", Group: GroupQuota, Kind: "int",
		Default:     "0",
		Description: `Per-token cap on successful chat requests per Pacific day (0 = unlimited, the default — the upstream quota/429 is the real enforcement). When set, locks the account until the daily quota reset (Pacific midnight, same instant upstream rolls its windows); capped tokens are skipped and the pool rolls to the next account. Set a value to bound a runaway loop locally.`},
	{Key: "MAX_REQUESTS_PER_MINUTE", Group: GroupQuota, Kind: "int",
		Default:     "0",
		Description: `Per-token cap on admitted chat requests per rolling 60s window (0 = unlimited, the default — the upstream quota/429 is the real enforcement). When set, throttles upstream-visible request bursts; a runaway loop locks within a minute and the pool rolls to the next account. Set a value to bound a runaway loop locally.`},
	{Key: "MAX_SPEND_PER_DAY", Group: GroupQuota, Kind: "int",
		Default:     "0",
	...
	{Key: "MODELS_HIDE_UNAVAILABLE", Group: GroupUpstream, Kind: "bool",
		Default:     "false",
	...
	{Key: "REGISTRY_REFRESH", Group: GroupUpstream, Kind: "text",
		Default:     "6h",
	...
	{Key: "CORS_ALLOWED_ORIGIN", Group: GroupSecurity, Kind: "text",
		Default:     "*",
		Description: `Access-Control-Allow-Origin for /v1/* responses.`},
	{Key: "DASHBOARD_REQUIRE_LOGIN", Group: GroupSecurity, Kind: "bool",
		Default:     "true",
