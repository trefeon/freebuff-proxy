...
var keyCatalog = []KeyDef{
	...
	{Key: "BURST_BALANCE_ENABLED", Group: GroupPool, Kind: "bool",
		Default:     "false",
		Description: `Spread one model's request burst across up to BURST_MAX_TOKENS accounts once BURST_THRESHOLD same-model admissions land inside BURST_WINDOW (ADR-0023, opt-in). Applies live on reload; false restores drain-only selection. ANTI-BAN TRADEOFF: spreading looks less like single-user traffic than drain — keep off unless one model's bursts throttle a single account while siblings with the same model sit idle.`},
	...
	{Key: "MATURITY_TOUCH_MODEL", Group: GroupPool, Kind: "text", Hidden: true,
		Default:     "deepseek/deepseek-v4-flash",
		Description: `Model the Economy maturity touch starts (cheapest served metered model — minimal daily Freebucks spend, never a premium model for farming).`},
