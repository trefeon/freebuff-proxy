...
var keyCatalog = []KeyDef{
	...
	{Key: "MATURITY_DRY_RUN", Group: GroupPool, Kind: "bool",
		Default:     "true",
		Description: `Maturity touches run only the zero-cost quota probe and never start a billable touch (scheduler mechanics validation). Set false after the dry-run log lines prove scheduling, skips and throttles behave.`},
	{Key: "MATURITY_ENABLED", Group: GroupPool, Kind: "bool", Essential: true,
		Default:     "true",
		Description: `Global kill-switch for streak-maturity automation: daily low-cost Freebucks touches that keep locked tokens' streaks alive until their target. Default true (probes run in dry-run mode); set false to disable all automated touches.`},
	...
	{Key: "MATURITY_TOUCH_MODEL", Group: GroupPool, Kind: "text",
		Default:     "auto",
		Description: `Touch model for unmetered maturity touches: "auto" (default) resolves the cheapest served unmetered row and falls back to this key's explicit value when no unmetered served row exists; an explicit provider/model id overrides auto. The touch never spends Freebucks (fail-closed on priced rows).`},
