...
var keyCatalog = []KeyDef{
	...
	{Key: "MATURITY_ALLOW_PREMIUM", Group: GroupPool, Kind: "bool", Hidden: true,
		Default:     "false",
		Description: `Allow the per-token premium-short touch mode (one short premium admission instead of the unmetered touch). Default false: premium touches burn daily quota and stay opt-in per token.`},
	{Key: "MATURITY_DRY_RUN", Group: GroupPool, Kind: "bool", Hidden: true,
		Default:     "true",
		Description: `Maturity touches run only the zero-cost session probe and never claim a session slot (scheduler mechanics validation). Set false after the dry-run log lines prove slots, skips and throttles behave.`},
	{Key: "MATURITY_ENABLED", Group: GroupPool, Kind: "bool", Essential: true,
		Default:     "false",
		Description: `Global kill-switch for streak-maturity automation: daily low-cost touches that keep locked tokens' streaks alive until their target. Default OFF (bot-shaped traffic must be explicit opt-in); per-token toggles on the Tokens page do nothing while this is off.`},
	...
	{Key: "MATURITY_TOUCH_MODEL", Group: GroupPool, Kind: "text", Hidden: true,
		...
		Description: `Unmetered model the maturity touch admits (must be a served, non-premium, uncapped catalog row — never burns premium quota for farming).`},
