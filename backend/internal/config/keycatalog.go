...
var keyCatalog = []KeyDef{
	...
	{Key: "MATURITY_ENABLED", Group: GroupPool, Kind: "bool", Essential: true,
		Default:     "true",
		Description: `Global kill-switch for the nightly streak-maintenance run: enrolled locked tokens get one low-cost touch in the 60 minutes before the Pacific-midnight reset until they reach MATURITY_TARGET_DAYS. Default true (probes run in dry-run mode); set false to disable all automated touches.`},
	{Key: "MATURITY_TARGET_DAYS", Group: GroupPool, Kind: "int",
		Default:     "7",
		Description: `Global streak target for every enrolled token (1-28 days; the dashboard enrolls with target 0 = this default, per-account targets are gone). A token whose streak reaches the target auto-releases its administrative lock.`},
	{Key: "MATURITY_TOUCH_MODEL", Group: GroupPool, Kind: "text",
		Default:     "",
		Description: `Global touch model for the nightly run: empty (default, same as "auto") admits the cheapest served price-0/exempt row per account and falls back to this key's explicit value when no unmetered served row exists; an explicit provider/model id overrides auto. The touch never spends Freebucks (fail-closed on priced rows). Accounts with client traffic since the last Pacific reset are skipped, as are quarantined/banned/cooling/country-blocked ones.`},
