...
- `toolnames_gen.go` is generated by `cmd/wiregen` (`wirefacts` package contract) from the verbatim wire snapshots — never hand-edit; regenerate via `go generate ./backend/internal/wirefacts/` and commit the result (CI `codegen-parity` enforces byte equality).
