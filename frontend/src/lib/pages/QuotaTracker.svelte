  ...
  {:else if data}
    {#if !data.has_tokens || !data.tokens?.length}
      <EmptyState
        ...
        description={$tr('Add a token to the pool to see per-model session quota and premium pool usage.')}
      />
    {:else}
      <div class="grid grid-cols-1 gap-4">
        {#each data.tokens as token, ti (token.index ?? ti)}
          ...
          <Card
            ...
            description={token.session_model ? $tr('Session: {model}', { model: token.session_model }) : ''}
          >
            <div class="flex flex-col gap-4">
              ...
              <div>
                <h3 class="text-xs font-semibold uppercase tracking-wider text-[var(--fp-muted)] mb-2">{$tr('Session quota by model')}</h3>
                {#if token.quota?.length}
                  <div class="overflow-x-auto">
                    <table class="fp-table">
                      <caption class="sr-only">{$tr('Session quota by model for token {index}', { index: idx })}</caption>
                      <thead>
                        <tr>
                      ...
                      <tbody>
                        {#each token.quota as q}
                          <tr>
                            <td><code class="fp-num text-xs {q.near_limit ? 'text-[#f5a623] font-medium' : 'text-[var(--fp-text)]'}">{q.model}</code></td>
                            <td class="fp-num text-xs {q.near_limit ? 'text-[#f5a623]' : 'text-[var(--fp-muted)]'}">{q.recent}</td>
                            <td class="fp-num text-xs {q.near_limit ? 'text-[#f5a623]' : 'text-[var(--fp-muted)]'}">{q.limit}</td>
                            <td class="fp-num text-xs {q.near_limit ? 'text-[#f5a623]' : 'text-[var(--fp-dim)]'}">{q.period}</td>
                            <td class="fp-num text-xs">
                              <span class={q.near_limit ? 'text-[#f5a623] font-medium' : 'text-[var(--fp-dim)]'}>
                                {formatLocalDate(q.reset_at_utc) || q.reset_at}{#if q.resets_in} ({q.resets_in}){/if}
                              </span>
                            </td>
                            <td class="fp-num text-xs {q.near_limit ? 'text-[#f5a623]' : 'text-[var(--fp-dim)]'}">{q.has_entitlement ? q.entitled : '—'}</td>
                          </tr>
                          {#if q.has_bar}
                            <tr>
                              <td colspan="6" class="!border-0 !pt-0 !pb-2">
                                <div
                                  class="h-1.5 w-full rounded-full overflow-hidden {q.near_limit ? 'bg-[#f5a623]/20' : 'bg-[var(--fp-inset)]'}"
                                  role="progressbar"
                                  aria-valuenow={q.usage_pct}
                                  aria-valuemin="0"
                                  aria-valuemax="100"
                                  aria-label={$tr('{model} session quota {pct}% used', { model: q.model, pct: q.usage_pct })}
                                >
                                  <div
      ...
      {#if bridgeQuotaCount > 0}
        <p class="text-xs text-[var(--fp-dim)]">
          {$tr('Bridge: {count} client(s) report premium quota — see the Tokens page for bridge details.', { count: bridgeQuotaCount })}
        </p>
      {/if}
    {/if}
  {/if}
