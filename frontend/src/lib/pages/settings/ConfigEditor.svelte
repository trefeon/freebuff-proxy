...
<div
  class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between pb-1"
>
  <!-- Mode Toggle -->
  <div
    class="flex items-center gap-1 p-1 rounded-lg bg-[var(--fp-surface)] border border-[var(--fp-border)]"
  >
    <button
      type="button"
      class="px-3 py-1 rounded-md text-xs font-medium transition-colors flex items-center gap-1.5 {viewMode ===
      'essential'
        ? 'bg-[var(--fp-accent)] text-[var(--fp-accent-fg,white)] font-semibold shadow-sm'
    ...
    >
      ...
      <span
        >{$tr("Essential ({count})", {
          count: meta.filter((e) => !e.hidden && isKeyEssential(e)).length,
        })}</span
      >
    </button>
    <button
    ...
    >
      ...
      <span
        >{$tr("All Settings ({count})", {
          count: meta.filter((e) => !e.hidden).length,
        })}</span
      >
    </button>
  </div>
...
<div class="space-y-6">
  ...
  {#if !groups.length}
    <Card>
      <EmptyState
        title={$tr("No matching configuration keys")}
        description={$tr(
          "No settings match the active search or category filter.",
        )}
      >
        {#snippet action()}
