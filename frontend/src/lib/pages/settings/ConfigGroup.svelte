...
<Card title={groupLabel(group.name)} description={groupDescription(group.name)}>
  <div class="divide-y divide-[var(--fp-border)]">
    {#each group.displayed as entry (entry.key)}
      <div
        class="py-3.5 first:pt-0 last:pb-0 grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-6 items-start"
      >
        <div class="md:col-span-6 min-w-0">
          <div class="flex flex-wrap items-center gap-2">
            <span
              id={`setting-${entry.key}`}
              class="fp-num text-[12px] font-semibold text-[var(--fp-text)]"
              >{entry.key}</span
            >
            {#if entry.restart_only}
              <span
                class="text-[10px] px-1.5 py-0.5 rounded-[var(--fp-radius-sm)] border border-[var(--fp-warning)]/40 bg-[var(--fp-warning)]/15 text-[#FCD34D] font-semibold uppercase tracking-wider shrink-0"
                >{$tr("restart")}</span
              >
            {/if}
            {#if entry.secret}
          ...
          {#if entry.description}
            <p class="mt-1 text-[11px] text-[var(--fp-dim)] leading-relaxed">
              {entry.description}
            </p>
          {/if}
        </div>
          ...
          {:else if entry.kind === "select"}
            ...
            >
              {#if entry.enum && !entry.enum.includes(formValues[entry.key])}
                <option value={formValues[entry.key]}
                  >{formValues[entry.key]}</option
                >
              {/if}
            ...
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                aria-label={reveal[entry.key]
                  ? $tr("Hide {key}", { key: entry.key })
                  : $tr("Reveal {key}", { key: entry.key })}
          ...
          {:else if entry.kind === "list"}
            <div>
              <textarea
                ...
                aria-label={entry.key}
                spellcheck="false"
                placeholder="val1, val2, val3"></textarea>
              <p class="mt-1 text-[10px] text-[var(--fp-dim)]">
  ...
  {#if !group.isExpanded && group.advanced.length > 0}
    <div
      class="pt-3 border-t border-[var(--fp-border)]/60 flex flex-col sm:flex-row sm:items-center justify-between gap-2"
    >
      <button
        type="button"
      ...
      >
        <ChevronDown size={14} />
        <span
          >{$tr("Show {count} more advanced settings in {group}", {
            count: group.advanced.length,
            group: groupLabel(group.name),
          })}</span
        >
      </button>
      <span
        class="text-[11px] text-[var(--fp-dim)] font-mono truncate max-w-xs"
      >
        {group.advanced
          .slice(0, 3)
          .map((e) => e.key)
          .join(", ")}{group.advanced.length > 3 ? "…" : ""}
      </span>
    </div>
